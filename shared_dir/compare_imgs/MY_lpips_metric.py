...
def main():
    ...
    for s in step:
        ...
        mask_detection = save_dir_name + '/' + 'estimation_mask/'
        if os.path.exists(mask_detection):
            shutil.rmtree(mask_detection)
        os.makedirs(mask_detection)
        
        mask_gt = save_dir_name + '/' + 'gt_mask/'
        if os.path.exists(mask_gt):
            shutil.rmtree(mask_gt)
        os.makedirs(mask_gt)
        
        blend = save_dir_name + '/' + 'blended/'
        if os.path.exists(blend):
            shutil.rmtree(blend)
        os.makedirs(blend)
        
        real_photo = save_dir_name + '/' + 'real/'
        if os.path.exists(real_photo):
            shutil.rmtree(real_photo)
        os.makedirs(real_photo)
        ...
        for i in tqdm(range(len(real_img_dir))):
            ...
            real_im = lpips.load_image(os.path.join(real_img_dir_step, real_img_dir[i]))
            fake_im = lpips.load_image(os.path.join(fake_img_dir_step, fake_img_dir[i]))
            depth_real = depth_apply(depth_img, real_im)
            depth_fake = depth_apply(depth_img, fake_im)
            real_img = lpips.im2tensor(depth_real)
            fake_img = lpips.im2tensor(depth_fake)
            ...
            comp = comp[0,0,:,:].data.cpu().numpy()  # comp.shape:(256, 256)
            heatmap = cv2.applyColorMap((comp.copy()*255).astype(np.uint8), cv2.COLORMAP_JET)
            heatmap = cv2.cvtColor(heatmap, cv2.COLOR_RGB2BGR)
            plt.imsave(save_dir_name + '/heatmap/' + (str)(i+1) + '_heatmap.png', heatmap)
            
            # アルファブレンディング
            alpha = 0.6
            blended = cv2.addWeighted(real_real_cut, alpha, heatmap, 1-alpha, 0)
            blended = cv2.cvtColor(blended, cv2.COLOR_BGR2RGB)
            heatmap = cv2.cvtColor(heatmap, cv2.COLOR_BGR2RGB)

            # anomaly_value 以上のところは少し赤くする
            det_mask = np.zeros((h, w)).astype(np.uint8)
            detection = np.zeros((h, w, 3))
            detection = real_real_cut.copy()
            for a in range(h):
                for n in range(w):
                    if comp[a][n] >= anomaly_value:
                        detection[a, n, 2] = real_real_cut[a, n, 2] + 90
                        det_mask[a][n] = det_mask[a][n] + 255
            cv2.imwrite(save_dir_name + '/' + 'detection/' + (str)(i+1) + '_detection.png', detection)
            cv2.imwrite(save_dir_name + '/' + 'estimation_mask/' + (str)(i+1) + '_est_mask.png', (det_mask))
            cv2.imwrite(save_dir_name + '/' + 'gt_mask/' + (str)(i+1) + '_gt_mask.png', (gt_mask_cut))
            cv2.imwrite(save_dir_name + '/' + 'real/' + (str)(i+1) + '_real.png', (real_real_cut))
            cv2.imwrite(save_dir_name + '/' + 'blended/' + (str)(i+1) + '_blended.png', blended)
            
            gt_mask_cut = cv2.cvtColor(gt_mask_cut, cv2.COLOR_GRAY2RGB)
            # matplotlib で5つの画像にして保存
            fig, axes = plt.subplots(1, 5)
            axes[0].imshow(real_real_cut[:,:,::-1])
            axes[1].imshow(gt_mask_cut)
            axes[2].imshow(heatmap[:,:,::-1])
            axes[3].imshow(blended[:,:,::-1])
            axes[4].imshow(det_mask)
            fig.set_size_inches(15, 3)
            fig.tight_layout()
            fig.savefig(save_dir_name + '/' + 'together/' + (str)(i+1) + '_together.png')
            
            gt_mask_cut = cv2.cvtColor(gt_mask_cut, cv2.COLOR_RGB2GRAY)
