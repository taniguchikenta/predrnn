...
parser.add_argument('--test_data_paths', type=str, default='data/moving-mnist-example/moving-mnist-test.npz')##################自作
parser.add_argument('--test_dir_name', type=str, default='test_dir_name')
...
def schedule_sampling(eta, itr):
    # 配列の全要素を 0 として定義する　引数は配列のshapeである　今回は五次元配列
    ...
    if itr < args.sampling_stop_iter:
        ...
        eta = 0.0
    # 0.0 以上 1.0 未満の乱数配列生成　引数は配列のshapeである　今回は引数が2つあるから二次元配列となっている
    random_flip = np.random.random_sample(
        (args.batch_size, args.total_length - args.input_length - 1))
    # 返り値は True or False
    true_token = (random_flip < eta)
    ...
    zeros = np.zeros((args.img_width // args.patch_size,
                      ...
                      args.patch_size ** 2 * args.img_channel))
    real_input_flag = []
    # real_input_flag にランダムに 0 1 を格納
    for i in range(args.batch_size):
...
def train_wrapper(model):
    ...
    train_input_handle, test_input_handle = datasets_factory.data_provider(
        args.dataset_name, args.train_data_paths, args.valid_data_paths, args.test_data_paths, args.test_dir_name, args.batch_size, args.img_width,##################### args.test_data_paths を追加
        seq_length=args.total_length, injection_action=args.injection_action, is_training=True)

    eta = args.sampling_start_value

    for itr in track(range(1, args.max_iterations + 1)):
...
def test_wrapper(model):
    model.load(args.pretrained_model)
    test_input_handle, frames_products_mark_handle = datasets_factory.data_provider(
        args.dataset_name, args.train_data_paths, args.valid_data_paths, args.test_data_paths, args.test_dir_name, args.batch_size, args.img_width,######################### args.test_data_paths を追加
        seq_length=args.total_length, injection_action=args.injection_action, is_training=False)
    trainer.test_test(model, test_input_handle, frames_products_mark_handle, args, 'test_result')##############################################
...
g = 0
while(i):
    print('GPU', g, ':', torch.cuda.get_device_name(g))
    i -= 1
    g += 1
